...
ClassifySmartPointerCapability(const CXXRecordDecl *Record);
  ...
  if (const auto *RT = VDT->getAs<RecordType>()) {
    if (const auto *RD = RT->getDecl()) {
      ...
      if (const auto *CXXRD = dyn_cast<CXXRecordDecl>(RD))
        if (auto DiagFromSmartPtr = ClassifySmartPointerCapability(CXXRD))
          return DiagFromSmartPtr.value();
    }
...
static std::optional<StringRef>
ClassifySmartPointerCapability(const CXXRecordDecl *Record) {
  auto FindOverloadedOperator =
      [](const CXXRecordDecl *Record,
         OverloadedOperatorKind Op) -> std::optional<const CXXMethodDecl *> {
  ...
  auto foundStarOperator = FindOverloadedOperator(Record, OO_Star);
  if (!foundStarOperator)
    return std::nullopt;

  auto foundArrowOperator = FindOverloadedOperator(Record, OO_Arrow);
  if (!foundArrowOperator)
    return std::nullopt;

  return ClassifyDiagnostic(foundStarOperator.value()->getReturnType());
}
...
CapabilityExpr SExprBuilder::translateAttrExpr(const Expr *AttrExp,
  ...
  if (const auto *OE = dyn_cast<CXXOperatorCallExpr>(AttrExp)) {
    ...
    if (UO->getOpcode() == UO_LNot) {
      ...
      //   AttrExp = CE->getSubExpr()->IgnoreImplicit();
    }
  }
...
// Make a Phi node in the current block for the i^th variable in
// CurrentVarMap. If E != null, sets Phi[CurrentBlockInfo->ArgIndex] = E. If E
// == null, this is a backedge and will be set later.
